...
class PPO_VQ(BaseAgent):
    ...
    def predict(self, obs, hidden_state, done):
        with torch.no_grad():
            ...
            hidden_state = torch.FloatTensor(hidden_state).to(device=self.device)
            mask = torch.FloatTensor(1 - done).to(device=self.device)
            dist, value, hidden_state, commit_loss = self.policy(obs, hidden_state, mask)
            act = dist.sample()
            log_prob_act = dist.log_prob(act)
    ...
    def optimize(self):
        pi_loss_list, value_loss_list, entropy_loss_list, x_ent_loss_list, total_loss_list, commit_loss_list = [], [], [], [], [], []
        batch_size = self.n_steps * self.n_envs // self.mini_batch_per_epoch
        if batch_size < self.mini_batch_size:
            self.mini_batch_size = batch_size
        grad_accumulation_steps = batch_size / self.mini_batch_size
        ...
        for e in range(self.epoch):
            ...
            for sample in generator:
                obs_batch, hidden_state_batch, act_batch, done_batch, \
                    old_log_prob_act_batch, old_value_batch, return_batch, adv_batch = sample
                mask_batch = (1 - done_batch)
                dist_batch, value_batch, _, commit_loss = self.policy(obs_batch, hidden_state_batch, mask_batch)
        ...
        summary = {'Loss/pi': np.mean(pi_loss_list),
                   ...
                   'Loss/entropy': np.mean(entropy_loss_list),
                   'Loss/x_entropy': np.mean(x_ent_loss_list),
                   'Loss/commit_loss': np.mean(commit_loss_list),
                   'Loss/total': np.mean(total_loss_list)}
        return summary
